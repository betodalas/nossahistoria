...
export default function AddMoment() {
  ...
  // Conta momentos locais para limite FREE (fallback)
  const localMoments = JSON.parse(localStorage.getItem('moments') || '[]')
  const FREE_LIMIT = 10
  const atLimit = !isPremium && localMoments.length >= FREE_LIMIT
  ...
  const handleVoiceSave = (audio: string, dur: number) => {
    setVoiceAudioB64(audio)
    setVoiceDuration(dur)
    // Converte base64 → Blob para enviar como arquivo
    try {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Foto como File real — campo esperado pelo backend: "photo"
      if (photoFile) formData.append('photo', photoFile)
      // Áudio como Blob — campo esperado pelo backend: "audio"
      ...
      // Fallback: salva localmente apenas se não for erro de autenticação/casal
      if (err?.response?.status === 401 || err?.response?.status === 403) {
        setError('Você precisa estar logado e vinculado a um casal para salvar momentos.')
        setSaving(false)
        return
      }
      const newMoment = {
